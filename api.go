...
func permissionDenied(w http.ResponseWriter, message ...string) {
	if len(message) > 0 {
		WriteJSON(w, http.StatusUnauthorized, ApiError{Error: message[0]})
		return
	}
...
func withJWTAuth(handlerFunc http.HandlerFunc, s Storage) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			permissionDenied(w, "token malformed")
			return
		}
		if !token.Valid {
		...
		if int64(claims["expiresAt"].(float64)) < time.Now().Local().Unix() {
...
func getUuid(idStr string) (uuid.UUID, error) {
	id, err := uuid.Parse(idStr)
	if err != nil {
		return id, fmt.Errorf("invalid id given %s", idStr)
	}
